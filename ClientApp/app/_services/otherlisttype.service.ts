...
import { OtherListType } from '../_models/index';
import { Subject } from 'rxjs/Subject';

@Injectable()
export class OtherListTypeService {
    token = '';
    ...
    insert(model: OtherListType) {
        const url = '/api/OtherListType/create';
        const body = {
            'name': model.name
        };
        const options = new RequestOptions({ headers: this.getHeaderWithToken(), url: url, body: body });
