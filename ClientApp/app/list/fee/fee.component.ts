...
export class ListFeeComponent implements OnInit, AfterViewInit {
    public state_type = 'new';
    // Declare variable search
    public obj_remove: Fee;
    public obj_edit: Fee;
    public obj_search: Fee;
    public list_data: Array<Fee> = [];
    @ViewChild('modalForm')
    form1: NgForm;
    @ViewChild('modal1')
    ...
    constructor(private dataService: FeeService, @Inject(DOCUMENT) private _document: any, private toastyService: ToastyService) {
        this._document.title = 'Danh sách chi phí';
    }
    public ngOnInit() {
        this.obj_remove = new Fee();
        this.obj_edit = new Fee();
        this.obj_search = new Fee();
        this.loadList();
    }
    ngAfterViewInit() {
    }
    ...
    public deleteRow(item: Fee): void {
